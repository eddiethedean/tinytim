...
class TestRowValueTuples(unittest.TestCase):
    def test_basic(self):
        ...
        results = utils.row_value_tuples(data, ['x', 'z'])
...
class TestRowDictsToData(unittest.TestCase):
    def test_basic(self):
        ...
        results = utils.row_dicts_to_data(rows)
...
class TestRowValuesGenerator(unittest.TestCase):
    def test_basic(self):
        row = {'x': 1, 'y': 4, 'z': 8}
        generator = utils.row_values_generator(row)
        v1 = next(generator)
...
class TestSliceToRange(unittest.TestCase):
    def test_basic(self):
        s = slice(0, 3, 1)
        r = utils.slice_to_range(s)
        self.assertEqual(r, range(0, 3, 1))

    def test_zero_step(self):
        s = slice(1, 4, 0)
        with self.assertRaises(ValueError):
            utils.slice_to_range(s)
...
class TestCombineNamesRows(unittest.TestCase):
    def test_basic(self):
        column_names = ['x', 'y']
        rows = ((1, 2), (4, 5), (8, 10))
        results = utils.combine_names_rows(column_names, rows)
